...
def euler_poisson():
    ...
    # Memory Allocation
    num_snapshot = 10 # number of snapshots
    num_snapstep = int(T / num_snapshot)
    ...
    n_mean = 3 / (4 * np.pi)
    k = 5 * 2 * np.pi / L
    n_0 = n_mean + 1e-4 * np.sin(k * x)
    u_0 = np.zeros(N) + 1e-4 * np.sin(k * x)
