...
def solve():
    ...
    # v[:] = np.ones(nx) + .25 * np.sin(3*IC_freq * X)
    # e[:] = np.ones(nx) + .25 * np.sin(3*IC_freq * X)

    n[:] = n_0 * np.ones(N) + .01 * np.random.random(N)
    ...
    for tt in range(1, T + 1):
        ...
        vrhs = vcorr - derivative(phi)  # TODO: LINE CHANGE
        erhs = derivative(np.log(n))*(therm_cond*derivative(e)+v*e)-v*derivative(e)
        ...
        if tt == int(T/100):
            ...
            print('Approximate total time:', elapsed_time * 100, 'seconds')
