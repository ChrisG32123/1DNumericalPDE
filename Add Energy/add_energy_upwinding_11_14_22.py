...
def solve():
    ...
    # Initial Conditions
    IC_freq = 2 * np.pi / L
    # n[:] = mean_n * np.ones(nx) + .02 * np.exp(-(X - Xlngth / 2) ** 2) - .02 * np.sqrt(np.pi)
    # v[:] = np.ones(nx) + .1 * np.sin(3*IC_freq * X)
    # e[:] = np.ones(nx) + .1 * np.sin(3*IC_freq * X)

    n[:] = n_0 * np.ones(N) + .2 * np.random.random(N)
    v[:] = np.ones(N) + .5 * np.random.random(N)
    e[:] = np.ones(N) + .5 * np.random.random(N)
    ...
    for tt in range(1, T + 1):
        ...
        vrhs = vcorr - n * derivative(phi)  # TODO: LINE CHANGE

        # Update
        n = n - lmbd * (nflux - l(nflux))
        v = v - lmbd * (vflux - l(vflux)) + dt * vrhs
        e = e - lmbd / dx * (r(eflux) - 2 * eflux + l(eflux))
    ...
    for ii in range(len(syssnaptot)):
        plt.figure()
        clr = plt.imshow(syssnaptot[ii, 0], aspect='auto', origin='lower')
        plt.colorbar()
        plt.title(syssnapname[ii][0] + " Gamma_0 = " + str(Gamma_0) + " kappa_0 = " + str(kappa_0))

    for ii in range(len(syssnapfluxtot)):
        plt.figure()
        clr = plt.imshow(syssnapfluxtot[ii, 0], aspect='auto', origin='lower')
        plt.colorbar()
        plt.title(syssnapfluxname[ii][0] + " Gamma_0 = " + str(Gamma_0) + " kappa_0 = " + str(kappa_0))

    plt.figure()
    clr = plt.imshow(phisnap[0], aspect='auto', origin='lower')
    plt.colorbar()
    ...
    for ii in range(len(nsnap)):
        fft = spfft.fft2(nsnap[ii] - np.mean(nsnap[ii]))
        fft = spfft.fftshift(fft)
