...
def solve():
    ...
    def d_dx(array):  # Checked
        return (array - l(array)) / dx

    def d2_dx2(array):  # Checked
    ...
    # Initial Conditions
    IC_freq = 2 * np.pi / L
    # n[:] = mean_n * np.ones(nx) + .01 * np.exp(-(X - Xlngth / 2) ** 2) - .01 * np.sqrt(np.pi)
    # n[:] = mean_n * np.ones(nx) + .01 * np.sin(3 * IC_freq * X)
    n[:] = n_0 * np.ones(N)
    v[:] = np.ones(N) + .01 * np.cos(3*IC_freq * x)
    e[:] = np.ones(N) + .01 * np.cos(3*IC_freq * x)

    # n[:] = mean_n * np.ones(nx) + .00001 * np.random.random(nx)
    # v[:] = np.ones(nx) + .00001 * np.random.random(nx)
    # e[:] = np.ones(nx) + .00001 * np.random.random(nx)

    phi = solvephi(n)

    nflux = dt * (d_dx(n * v))
    vflux = dt * (d_dx(e + .5 * v * v + phi) + d_dx(np.log(n)) * e)
    eflux = dt * (d_dx(v*e) + d_dx(v)*e - d2_dx2(therm_cond * e) - d_dx(np.log(n)) * d_dx(therm_cond*e))
    ...
    for tt in range(1, T + 1):
        ...
        ln, lv, le, lphi = l(n), l(v), l(e), l(phi)
        lnflux = dt * (d_dx(ln * lv))
        lvflux = dt * (d_dx(le + .5 * lv * lv + lphi) + d_dx(np.log(ln)) * le)
        leflux = dt * (d_dx(lv * le) + d_dx(lv) * le - d2_dx2(therm_cond * le) - d_dx(np.log(ln)) * d_dx(therm_cond * le))

        rn, rv, re, rphi = r(n), r(v), r(e), r(phi)
        rnflux = dt * (d_dx(rn * rv))
        rvflux = dt * (d_dx(re + .5 * rv * rv + rphi) + d_dx(np.log(rn)) * re)
        reflux = dt * (d_dx(rv * re) + d_dx(rv) * re - d2_dx2(therm_cond * re) - d_dx(np.log(rn)) * d_dx(therm_cond * re))

        # Solve Correlation
        vcorr = np.zeros((2, N))  # np.array([np.zeros(nx), fft_correlations(k,n[1], Gamma_0, kappa_0)])    # TODO: Add Correlations
        ...
        # Left Boundary
        n[:,0] = n[:,0] - nflux[:,0]
        v[:,0] = v[:,0] - vflux[:,0] + dt * vrhs[:,0]
        e[:,0] = e[:,0] - eflux[:,0] + dt * erhs[:,0]
    ...
    for ii in range(len(syssnaptot)):
        ...
        plt.title(syssnapname[ii][0] + " Γ0 = " + str(Gamma_0) + " κ_0 = " + str(kappa_0))
        plt.xlabel("Space")
        plt.ylabel(syssnapname[ii][0])
        plt.legend()
    ...
    for ii in range(len(syssnapfluxtot)):
        ...
        plt.title(syssnapfluxname[ii][0] + " Γ0 = " + str(Gamma_0) + " κ0 = " + str(kappa_0))
        plt.xlabel("Space")
        plt.ylabel(syssnapfluxname[ii][0])
        plt.legend()
    ...
    for jj in range(len(phisnap)):
        ...
        plt.title("phi" + " Γ0 = " + str(Gamma_0) + " κ0 = " + str(kappa_0))
        plt.legend()
    ...
    for ii in range(len(syssnapint)):
        ...
        plt.title(syssnapintname[ii][0] + " Integral: Γ0 = " + str(Gamma_0) + " κ0 = " + str(kappa_0))
        plt.xlabel("Time")
        plt.ylabel("Total " + syssnapintname[ii][0])
        plt.legend()
    ...
    for ii in range(len(syssnaptot)):
        ...
        plt.colorbar()
        plt.title(syssnapname[ii][0] + " Γ0 = " + str(Gamma_0) + " κ0 = " + str(kappa_0))
        plt.xlabel("Space")
        plt.ylabel("Time")
    ...
    plt.colorbar()
    plt.title("Phi, NC: Γ0 = " + str(Gamma_0) + " κ0 = " + str(kappa_0))
    plt.xlabel("Space")
    plt.ylabel("Time")
    ...
    #     plt.colorbar()
    #     plt.title("FFT: Γ0 = " + str(Γ0) + " κ0 = " + str(kappa_0))
    ...
    #         plt.title('Γ = ' + str(Γ[ii]) + ', κ = ' + str(kappa[jj]))
    #         plt.title('Γ = ' + str(Γ0) + ', κ = ' + str(kappa_0))
    #         plt.colorbar()
    #         plt.ylabel("Time")
    #         plt.xlabel("Space")
